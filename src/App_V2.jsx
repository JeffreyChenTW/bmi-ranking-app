...
export default function AppV2() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      if (headerRowIndex === -1) {
        setError("找不到欄位列，請確認有：名字 / 性別 / 身高 / Week1...");
        return;
      }
      ...
      const nameIdx = headerRow.findIndex((x) => {
  ...
  return (
    ...
    v === "name"
      ...
      const genderIdx = headerRow.findIndex((x) => {
  const v = String(x).trim().toLowerCase();
  ...
  return (
    ...
    v === "gender"
      ...
      const heightIdx = headerRow.findIndex((x) => {
  const v = String(x).trim().toLowerCase();
    ...
    v === "height"
      ...
      for (let i = 0; i < Math.max(weekRow.length, headerRow.length); i++) {
        ...
        if (/week\s*\d+/i.test(weekText)) {
          weekColumns.push({
            week: weekText.replace(/\s+/g, ""),
            index: i,
          });
        } else if (/week\s*\d+/i.test(headerText)) {
          weekColumns.push({
            week: headerText.replace(/\s+/g, ""),
            index: i,
          });
        }
      }
      ...
      if (!weekColumns.length) {
        setError("找不到 Week1 / Week2 / Week3 欄位");
        return;
      }
      ...
      const availableWeeks = weekColumns.map((w) => w.week);
  ...
  const buildRanking = (weekName, includeBonus) => {
    ...
    return records
      .map((r) => {
        ...
        if (baselineBMI == null || currentBMI == null) {
          return null;
        }

        const reduction =
          ((baselineBMI - currentBMI) / baselineBMI) * 100;
      ...
      <div style={styles.container}>
        <div style={styles.hero}>
          ...
          <div style={styles.subtitle}>
            {selectedWeek || "No Week Selected"} · Final Ranking with Bonus
          </div>
        ...
        <div style={styles.cardGrid}>
          ...
          <div style={styles.statCard}>
            ...
            <div style={styles.statValue}>{topScore}%</div>
          </div>
        </div>
